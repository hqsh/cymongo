...
data_frame_data_t * find (mongoc_collection_t *collection, data_frame_info_t *data_frame_info, bool debug)
{
    ...
    while (mongoc_cursor_next (cursor, &doc)) {
        if (bson_iter_init (&index_iter, doc) && bson_iter_find (&index_iter, data_frame_info->index_key) &&
            ...
            _PROCESS_INDEX_OR_COLUMN (index_type, index_iter, index_key, p_mongo_data->index_chain_head, p_mongo_data->string_index_max_length)
            _PROCESS_INDEX_OR_COLUMN (column_type, column_iter, column_key, p_mongo_data->column_chain_head, p_mongo_data->string_column_max_length)
            is_first_doc = false;
        }
    }
    bson_destroy (query);
    ...
    _CREATE_INDEX_OR_COLUMN_ARRAY (data_frame_info->index_type, p_mongo_data->index_chain_head,
                                   p_data_frame_data->row_cnt, p_mongo_data->string_index_max_length,
    ...
    _CREATE_INDEX_OR_COLUMN_ARRAY (data_frame_info->column_type, p_mongo_data->column_chain_head,
                                   p_data_frame_data->col_cnt, p_mongo_data->string_column_max_length,
